...
const STATUS = {
  ...
  upload_finish: {
    value: "upload_finish",//上传完成
    desc: "上传完成",
    color: "#67c23a",
    icon: "ok",
  },
  upload_seconds: {
    value: "upload_seconds",//上传完成（秒传）
    desc: "秒传",
    color: "#67c23a",
    icon: "ok",
  },
...
const uploadFile = async (uid, chunkIndex) => {
  ...
  for (let i = chunkIndex; i < chunks; i++) {
    ...
    if (delIndex != -1) {//上传过程中,文件被删除
      delList.value.splice(delIndex, 1);
      // console.log(delList.value);
      break;
    }
    currentFile = getFileByUid(uid);
    if (currentFile.pause) {//上传过程中，文件被暂停
      break;
    }
    let start = i * chunkSize;
    let end = start + chunkSize >= fileSize ? fileSize : start + chunkSize;
    let chunkFile = file.slice(start, end);//截取文件的分片
    let uploadResult = await proxy.Request({//上传
      url: api.upload,
      showLoading: false,
      dataType: "file",
      params: {
      ...
      errorCallback: (errorMsg) => {
        currentFile.status = STATUS.fail.value;//上传失败
  ...
  return new Promise((resolve, reject) => {
    ...
    fileReader.onload = (e) => {
      ...
      if (currentChunk < chunks) {
        ...
        spark.destroy(); //释放缓存
        resultFile.md5Progress = 100;//md5计算完成
        resultFile.status = STATUS.uploading.value;//状态由 init => uploading
        resultFile.md5 = md5;
        resolve(fileItem.uid);
      }
    };
    fileReader.onerror = () => {
      resultFile.md5Progress = -1;//解析失败
      resultFile.status = STATUS.fail.value;//状态由 init => fail
      resolve(fileItem.uid);
    };
  }).catch((error) => {
    return null;
